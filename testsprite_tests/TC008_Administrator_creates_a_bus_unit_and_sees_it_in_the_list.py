...
async def run_test():
    ...
    try:
        ...
        # -> Click the 'Unidades' link (element index 136) to navigate to the units management page.
        # link "Unidades"
        elem = page.locator("xpath=/html/body/div/div/aside/nav/a[5]").nth(0)
        await elem.wait_for(state="visible", timeout=10000)
        await elem.click()
        
        # -> Click the 'Nueva unidad' button (element index 1118) to open the create-unit form/modal.
        # button "Nueva unidad"
        elem = page.locator("xpath=/html/body/div/div/main/div/div/button").nth(0)
        await elem.wait_for(state="visible", timeout=10000)
        await elem.click()
        
        # -> Fill the Placa field with 'ABC123' (index 1168), the Nombre field with 'Unidad Central' (index 1170), then click 'Crear unidad' (index 1173) to submit the form.
