...
async def run_test():
    ...
    try:
        ...
        # -> Fill the email and password fields with provided credentials and click the Ingresar (submit) button to log in.
        ...
        # -> Click the 'Tarifario' link in the left navigation to open the fare/pricing list and load the Tarifario page.
        # link "Tarifario"
        elem = page.locator("xpath=/html/body/div/div/aside/nav/a[6]").nth(0)
        await elem.wait_for(state="visible", timeout=10000)
        await elem.click()
        
        # -> Open the fare creation/edit form by clicking the 'Nueva tarifa' button (interactive element index 1142).
        # button "Nueva tarifa"
        elem = page.locator("xpath=/html/body/div/div/main/div/div/button").nth(0)
        await elem.wait_for(state="visible", timeout=10000)
        await elem.click()
        
        # -> Enter a new price (3.00) into the Precio (S/) field (index 1213) and click 'Guardar tarifa' (index 1219) to save the fare.
        # number input
        elem = page.locator("xpath=/html/body/div[3]/form/div[2]/input").nth(0)
        await elem.wait_for(state="visible", timeout=10000)
        await elem.fill("3.00")
