...
async def run_test():
    ...
    try:
        ...
        # -> Fill the email and password fields with the administrator credentials and click the 'Ingresar' submit button to sign in.
        ...
        # -> Click the 'Unidades' link (interactive element [136]) to open the Units management page.
        # link "Unidades"
        elem = page.locator("xpath=/html/body/div/div/aside/nav/a[5]").nth(0)
        await elem.wait_for(state="visible", timeout=10000)
        await elem.click()
        
        # -> Open the create-new-unit form by clicking the 'Nueva unidad' button (index 1142).
        # button "Nueva unidad"
        elem = page.locator("xpath=/html/body/div/div/main/div/div/button").nth(0)
        await elem.wait_for(state="visible", timeout=10000)
        await elem.click()
        
        # -> Fill the 'Placa' field with 'GHI-789' and the 'Nombre / descripción' field with 'Automated Bus 2026-05-25', then submit the form by clicking the 'Crear unidad' button.
        # text input placeholder="Ej: ABC-123"
        elem = page.locator("xpath=/html/body/div[3]/form/div/input").nth(0)
        await elem.wait_for(state="visible", timeout=10000)
        await elem.fill("GHI-789")
        
        # -> Fill the 'Placa' field with 'GHI-789' and the 'Nombre / descripción' field with 'Automated Bus 2026-05-25', then submit the form by clicking the 'Crear unidad' button.
        # text input placeholder="Ej: Bus Norte 01"
        elem = page.locator("xpath=/html/body/div[3]/form/div[2]/input").nth(0)
        await elem.wait_for(state="visible", timeout=10000)
        await elem.fill("Automated Bus 2026-05-25")
