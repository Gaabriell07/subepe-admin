...
async def run_test():
    ...
    try:
        ...
        # -> Fill the administrator email and password fields and submit the login form (input index 4 -> email, input index 5 -> password, click index 7).
        ...
        # -> Click the 'Unidades' link (interactive element index 136) to open the units management page.
        # link "Unidades"
        elem = page.locator("xpath=/html/body/div/div/aside/nav/a[5]").nth(0)
        await elem.wait_for(state="visible", timeout=10000)
        await elem.click()
        
        # -> Click the 'Nueva unidad' button (index 1118) to open the create-unit form so its plate and name fields can be filled.
        # button "Nueva unidad"
        elem = page.locator("xpath=/html/body/div/div/main/div/div/button").nth(0)
        await elem.wait_for(state="visible", timeout=10000)
        await elem.click()
        
        # -> Fill the plate with 'DEF-456', fill the name with 'Bus Test 02', and submit the form by clicking the Crear unidad button.
        # text input placeholder="Ej: ABC-123"
        elem = page.locator("xpath=/html/body/div[3]/form/div/input").nth(0)
        await elem.wait_for(state="visible", timeout=10000)
        await elem.fill("DEF-456")
        
        # -> Fill the plate with 'DEF-456', fill the name with 'Bus Test 02', and submit the form by clicking the Crear unidad button.
        # text input placeholder="Ej: Bus Norte 01"
        elem = page.locator("xpath=/html/body/div[3]/form/div[2]/input").nth(0)
        await elem.wait_for(state="visible", timeout=10000)
        await elem.fill("Bus Test 02")
